...
{
	int size = 0;
	size = s.port.length() + to_string(s.stationNum).length() + s.id.length() + to_string(s.size).length() + msg.length() + 5;
	return size;
}
...
std::string get_path()
{
	...
	{
		cerr << "Can't connect to server, Err # \n--------PLEASE MAKE SURE THE SERVER IS RUNNING--------" << WSAGetLastError() << endl;
		closesocket(sock);
		WSACleanup();
		exit(0);
	}
	...
	if (userInput.size() > 0) // Make sure the user has typed in something
	{
		...
		if (sendResult != SOCKET_ERROR)
		{
			...
			if (bytesReceived > 0)
			{
				...
				std::cout << "Client> " << string(buf, 0, bytesReceived) << endl;
				path = string(buf, 0, bytesReceived);
			}
		}
	}
